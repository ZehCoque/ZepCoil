...
router.get('/contagem_contratos_alerta', function (req, res, next) {
  auth.db_conn().getConnection((err,connection) => {
    ...
    let database = connection.config.database + '.';
    connection.query(
      'SELECT COUNT(Identificacao) as Contagem from ' + database + 'alerta_contratos',
      [],
      (error, results) => {
        if (error) {
          console.log(error);
  ...
  router.get('/contagem_contratos_alerta_6_meses', function (req, res, next) {
    auth.db_conn().getConnection((err,connection) => {
      ...
      let database = connection.config.database + '.';
      connection.query(
        'SELECT COUNT(Identificacao) as Contagem FROM ' + database + ' alerta_final_contrato',
        [],
        (error, results) => {
          if (error) {
            console.log(error);
...
router.get('/contratos_alerta', function (req, res, next) {
  ...
  router.get('/contratos_alerta_6_meses', function (req, res, next) {
    auth.db_conn().getConnection((err,connection) => {
      ...
      let database = connection.config.database + '.';
      connection.query(
        'SELECT * FROM ' + database + 'alerta_final_contrato',
        [],
        (error, results) => {
          if (error) {
            console.log(error);
